...
class BugPersistenceIntegrationTest extends InfrastructureIntegrationTestBase {
    ...
    @BeforeEach
    void seedScope() {
        jdbc.update("DELETE FROM work_item_events WHERE organization_id=2903");
        jdbc.update("DELETE FROM work_item_relations WHERE organization_id=2903");
        jdbc.update("DELETE FROM bug_details WHERE organization_id=2903");
        jdbc.update("DELETE FROM test_results WHERE organization_id=2903");
        jdbc.update("DELETE FROM test_runs WHERE organization_id=2903");
        jdbc.update("DELETE FROM test_cases WHERE organization_id=2903");
        jdbc.update("DELETE FROM work_items WHERE organization_id=2903");
        jdbc.update("DELETE FROM organization_item_sequences WHERE organization_id=2903");
        jdbc.update("DELETE FROM organizations WHERE id=2903");
    ...
    void createsRelationsThenPersistsFixVerifyAndReopenHistory() {
        WorkItem item = workItems.create(2903, 2901, WorkItemType.BUG, "Checkout fails", "HTTP 500",
        ...
        BugView fixing = bugs.transition(2903, created.id(), 2901, BugAction.START_FIX,
                "IN_PROGRESS", "", List.of(), 0, "bug-start");
        BugView resolved = bugs.transition(2903, created.id(), 2901, BugAction.RESOLVE,
                "RESOLVED", "Handle null cart", List.of("mr://42", "commit://abc"), fixing.version(), "bug-resolve");
        BugView verified = bugs.transition(2903, created.id(), 2901, BugAction.VERIFY,
                "VERIFIED", "Retested", List.of(), resolved.version(), "bug-verify");
        BugView reopened = bugs.transition(2903, created.id(), 2901, BugAction.REOPEN,
