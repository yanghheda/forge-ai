...
public interface DeploymentMapper {
    ...
    @Insert("INSERT INTO deployments (workspace_id,project_id,release_id,mode,status,requested_by,"
            + "approval_expires_at,release_version,precheck_id,argument_hash,simulate_failure,idempotency_key,"
            + "approval_source,agent_approval_id,created_at,version) VALUES (#{workspaceId},#{projectId},"
    ...
    @Select("SELECT * FROM deployments WHERE id=#{deploymentId} AND workspace_id=#{workspaceId} "
            + "AND project_id=#{projectId}")
    List<Map<String, Object>> find(@Param("workspaceId") long workspaceId,
            @Param("projectId") long projectId, @Param("deploymentId") long deploymentId);

    @Select("SELECT * FROM deployments WHERE workspace_id=#{workspaceId} AND project_id=#{projectId} "
    ...
    List<Map<String, Object>> findByIdempotencyKey(@Param("workspaceId") long workspaceId,
            @Param("projectId") long projectId, @Param("idempotencyKey") String idempotencyKey);
    ...
    @Update("UPDATE deployments SET status=#{status},approver_user_id=#{approverUserId},"
            ...
            + "AND workspace_id=#{workspaceId} AND project_id=#{projectId} AND status='PENDING_APPROVAL' "
    ...
    @Update("UPDATE deployments SET status='EXPIRED',decided_at=UTC_TIMESTAMP(6),version=version+1 "
            + "WHERE id=#{deploymentId} AND workspace_id=#{workspaceId} AND project_id=#{projectId} "
    ...
    int updateReleaseStatus(@Param("workspaceId") long workspaceId, @Param("projectId") long projectId,
    ...
    @Insert("INSERT INTO work_item_events (workspace_id,project_id,work_item_id,event_type,from_status,to_status,"
            + "actor_type,actor_id,reason,metadata_json,idempotency_key,created_at) SELECT #{workspaceId},"
            + "#{projectId},item.id,#{eventType},#{fromStatus},#{toStatus},'SYSTEM',#{actorId},"
            ...
            + "AND ri.workspace_id=#{workspaceId} AND ri.project_id=#{projectId} AND item.workspace_id=#{workspaceId} "
            + "AND item.project_id=#{projectId} AND item.type='REQUIREMENT' AND item.status=#{fromStatus}")
    int insertRequirementEvents(@Param("workspaceId") long workspaceId, @Param("projectId") long projectId,
    ...
    @Update("UPDATE work_items item JOIN release_items ri ON ri.work_item_id=item.id "
            ...
            + "AND item.workspace_id=#{workspaceId} AND item.project_id=#{projectId} "
            + "AND item.type='REQUIREMENT' AND item.status=#{fromStatus}")
    int transitionRequirements(@Param("workspaceId") long workspaceId, @Param("projectId") long projectId,
    ...
    @Insert("INSERT INTO audit_logs (workspace_id,project_id,actor_type,actor_id,action,resource_type,resource_id,"
            + "result,request_id,run_id,metadata_redacted_json,created_at) VALUES (#{workspaceId},#{projectId},"
            + "#{actorType},#{actorId},#{action},'DEPLOYMENT',#{deploymentId},#{result},#{requestId},#{runId},"
            + "JSON_OBJECT('mode','SIMULATED'),UTC_TIMESTAMP(6))")
    int audit(@Param("workspaceId") long workspaceId, @Param("projectId") long projectId,
