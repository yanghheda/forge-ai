...
public interface WebhookMapper {
    ...
    @Select("SELECT c.id connection_id,c.workspace_id,s.type,s.ciphertext,s.iv,s.key_version,s.fingerprint "
            + "FROM gitlab_connections c JOIN secrets s ON s.id=c.webhook_secret_id AND s.workspace_id=c.workspace_id "
    ...
    @Insert("INSERT IGNORE INTO webhook_deliveries "
            + "(workspace_id,connection_id,delivery_key,event_type,payload_hash,payload,status,attempts,next_attempt_at,received_at) "
            + "SELECT workspace_id,id,#{deliveryKey},#{eventType},#{payloadHash},CAST(#{payload} AS JSON),'PENDING',0,UTC_TIMESTAMP(6),UTC_TIMESTAMP(6) "
    ...
    @Select("SELECT id,workspace_id,connection_id,delivery_key,event_type,payload_hash,payload,attempts,received_at "
    ...
    @Select("SELECT r.id,r.workspace_id FROM git_repositories r WHERE r.connection_id=#{connectionId} "
    ...
    @Select("SELECT remote_updated_at FROM merge_requests WHERE workspace_id=#{workspaceId} "
            + "AND repository_id=#{repositoryId} AND remote_mr_iid=#{remoteId} FOR UPDATE")
    List<java.time.LocalDateTime> lockMergeRequestTime(@Param("workspaceId") long workspaceId,
            @Param("repositoryId") long repositoryId, @Param("remoteId") long remoteId);

    @Select("SELECT remote_updated_at FROM pipeline_runs WHERE workspace_id=#{workspaceId} "
            + "AND repository_id=#{repositoryId} AND remote_pipeline_id=#{remoteId} FOR UPDATE")
    List<java.time.LocalDateTime> lockPipelineTime(@Param("workspaceId") long workspaceId,
            @Param("repositoryId") long repositoryId, @Param("remoteId") long remoteId);
    ...
    @Insert("INSERT INTO merge_requests (workspace_id,repository_id,work_item_id,remote_mr_iid,title,source_branch,"
            ...
            + "VALUES (#{workspaceId},#{repositoryId},NULL,#{iid},#{title},#{source},#{target},#{state},#{webUrl},"
    ...
    int upsertMergeRequest(@Param("workspaceId") long workspaceId, @Param("repositoryId") long repositoryId,
    ...
    @Insert("INSERT INTO pipeline_runs (workspace_id,repository_id,merge_request_id,remote_pipeline_id,ref,commit_sha,"
            ...
            + "VALUES (#{workspaceId},#{repositoryId},NULL,#{pipelineId},#{ref},#{sha},#{status},#{webUrl},#{startedAt},"
            ...
            + "last_synced_at=IF(remote_updated_at<VALUES(remote_updated_at),UTC_TIMESTAMP(6),last_synced_at),"
            + "remote_updated_at=GREATEST(remote_updated_at,VALUES(remote_updated_at))")
    int upsertPipeline(@Param("workspaceId") long workspaceId, @Param("repositoryId") long repositoryId,
