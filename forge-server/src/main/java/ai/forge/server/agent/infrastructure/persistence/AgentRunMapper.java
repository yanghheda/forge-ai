...
public interface AgentRunMapper {
    ...
    @Insert("INSERT INTO agent_runs (id, organization_id, work_item_id, user_id, skill, "
            ...
            + "updated_at) VALUES "
            + "(#{runId}, #{organizationId}, #{workItemId}, #{userId}, #{skill}, "
    ...
    @Select("SELECT id, organization_id, work_item_id, user_id, skill, medium_tool_confirmation, status, "
    ...
    @Select("SELECT s.step_no, s.type, s.name, s.status, s.output_summary, s.started_at, s.finished_at FROM agent_steps s "
            + "JOIN agent_runs r ON r.id = s.run_id WHERE r.organization_id = #{organizationId} "
            + "AND r.organization_id = #{organizationId} AND r.id = #{runId} ORDER BY s.step_no")
    List<Map<String, Object>> findSteps(
            @Param("organizationId") long organizationId,
            @Param("runId") String runId);

    @Select("SELECT e.run_id, e.sequence, e.event_type, e.request_id, e.payload_json, e.created_at "
            + "FROM agent_events e JOIN agent_runs r ON r.id = e.run_id WHERE r.organization_id = #{organizationId} "
            + "AND r.organization_id = #{organizationId} AND r.id = #{runId} AND e.sequence > #{afterSequence} "
    ...
    @Select("SELECT status, last_sequence FROM agent_runs WHERE id = #{runId} AND organization_id = #{organizationId} "
    ...
    @Update("UPDATE agent_runs SET status = 'RUNNING', started_at = UTC_TIMESTAMP(6), last_sequence = #{lastSequence}, "
            ...
            + "AND organization_id = #{organizationId} AND status = 'QUEUED'")
    int markRunning(
            @Param("organizationId") long organizationId,
            @Param("runId") String runId,
