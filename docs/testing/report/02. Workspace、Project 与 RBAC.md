...
实际/证据：未加入 某project的 成员 ，手动切换 该 project url后，能正常访问页面，没拒绝，但是 数据一直处在加载中。修复方向应为跳回首页。
...
实际/证据：保存 prd 内容后，前端没地方看 prd 内容。还有 requirement prd uxspec 文档 默认应该是 读的状态而不是直接是编辑的状态，点击编辑按钮才进入编辑状态
