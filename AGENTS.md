...
- 开发严格遵守当前会话范围。会话编号、主题与对应设计章节通过 `docs/context/INDEX.md` 定位。
- 不为方便而提前实现后续会话内容；发现跨阶段问题时记录为遗留项。
- 设计若改变事实来源、认证、事务、Agent Tool 安全边界或部署拓扑，必须先新增 ADR。

## 上下文路由

- 代码实施、设计调整或复盘任务开始时，先读取 `docs/context/INDEX.md`；纯命令执行、状态查询或与项目设计无关的任务无需读取。
- 用户给出会话编号时，以该编号路由为准；未给出时，根据任务主题和目标目录选择最小路由。
- 只读取路由命中的章节及直接相关代码，不得默认通读 PRD、完整技术方案、详细设计或分阶段开发指南。
- 多目标任务必须先建立“目标 → 会话/主题 → 章节”映射，合并去重后读取；若命中不同会话或跨阶段内容，先说明范围冲突，不自行扩大实施范围。
- 只在索引无法覆盖、命中章节存在冲突、用户要求全局评审，或需要改变架构基线时，才扩大读取范围。
...
- 所有公司业务 Mapper SQL 必须显式携带适用的 `organization_id` 条件；改用 MyBatis 不得弱化授权边界。GitLab 自身资源标识（如 `remote_project_id`）不属于 ForgeAI 业务作用域。
