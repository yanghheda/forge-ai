...
import { completeTestRun, createBug, createTestCase, createTestRun, getLatestTestRun, listBugs, listTestCases, reopenTestRun, updateTestResult, transitionBug, type BugView, type TestRunView } from "../api/work-item-api";
...
export function QaPanel({ organizationId, requirementId, onChanged }: { organizationId: number; requirementId: number; onChanged: () => Promise<unknown> }) {
  ...
  const cases = useQuery({
    queryKey: [...key, "cases"],
    queryFn: () => listTestCases(organizationId, requirementId),
  });
  const run = useQuery({
    queryKey: [...key, "run"],
    queryFn: () => getLatestTestRun(organizationId, requirementId),
    retry: false,
  });
  const bugs = useQuery({
    queryKey: [...key, "bugs"],
    queryFn: () => listBugs(organizationId, requirementId),
  });
  ...
  const moveBug = useMutation({
    mutationFn: ({ bug, action, reason, evidence }: { bug: BugView; action: "START_FIX" | "RESOLVE" | "VERIFY" | "CLOSE" | "REOPEN"; reason?: string; evidence?: string }) =>
      transitionBug({
        organizationId,
        bugId: bug.id,
        action,
    ...
    <Card title="QA 验证">
      <Space direction="vertical" style={{ width: "100%" }}>
        ...
        <Space>
          ...
          <Button disabled={!title.trim()} onClick={() => createCase.mutate()}>
            创建用例
          </Button>
          <Button disabled={!cases.data?.length} onClick={() => createRun.mutate()}>
            创建测试执行
          </Button>
        </Space>
...
export function QaRunView({
  ...
  return (
    <Space direction="vertical" style={{ width: "100%" }}>
      <Space>
        <Tag>{run.status}</Tag>
        <Typography.Text>{run.environment}</Typography.Text>
      </Space>
      {run.results.map((result) => (
        <Card key={result.id} size="small" title={`${result.priority} · ${result.title}`}>
          <Select aria-label={`${result.title} 结果`} value={result.status} disabled={busy || run.status === "COMPLETED"} onChange={(status) => status !== "NOT_RUN" && onResult(result.id, status, result.version)}>
            {["NOT_RUN", "PASS", "FAIL", "BLOCKED", "SKIPPED"].map((status) => (
              <Select.Option key={status} value={status}>
                {testResultLabel(status)}
              </Select.Option>
            ))}
          </Select>
      ...
      {run.status === "COMPLETED" ? (
        ...
        <Button disabled={run.results.some((result) => result.status === "NOT_RUN")} onClick={onComplete}>
          完成测试执行
        </Button>
      )}
    </Space>
...
export function BugList({ bugs, onAction }: { bugs: BugView[]; onAction: (bug: BugView, action: "START_FIX" | "RESOLVE" | "VERIFY" | "CLOSE" | "REOPEN", reason?: string, evidence?: string) => void }) {
  ...
  const nextAction = (status: BugView["status"]) =>
    ...
    )[status as "OPEN" | "IN_PROGRESS" | "RESOLVED" | "VERIFIED" | "CLOSED"];
  return (
    <Space direction="vertical" style={{ width: "100%" }}>
          ...
          <Card key={bug.id} size="small" title={`${bug.itemKey} · ${bug.title}`}>
            <Space>
              ...
              {bug.status === "IN_PROGRESS" && (
                <>
                  <Input aria-label={`${bug.itemKey} 修复说明`} placeholder="修复说明" value={fixNotes[bug.id] ?? ""} onChange={(value) => setFixNotes((current) => ({ ...current, [bug.id]: value }))} />
                  <Input aria-label={`${bug.itemKey} 修复证据`} placeholder="MR 或 Commit 引用" value={fixEvidence[bug.id] ?? ""} onChange={(value) => setFixEvidence((current) => ({ ...current, [bug.id]: value }))} />
                </>
              )}
              {action && (
                <Button disabled={action === "RESOLVE" && (!fixNotes[bug.id]?.trim() || !fixEvidence[bug.id]?.trim())} onClick={() => onAction(bug, action, fixNotes[bug.id], fixEvidence[bug.id])}>
