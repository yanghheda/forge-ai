      ...
      {tasks.map((task) => (
        ...
        >
          <span className={ui.itemMain}>
            ...
            <span className={ui.itemMeta}>{task.itemKey} · {priorityLabel(task.priority)}</span>
          </span>
  ...
  return (
    <section className={ui.page}>
      <header className={ui.pageHeader}>
        <div><span className={ui.eyebrow}>设计交付</span><h1>UX 工作台</h1><p>处理设计任务，提交 UX 交付物并跟进评审。</p></div>
      </header>
  ...
  return (
    <Card title={`${task.itemKey} · ${task.title}`} className={ui.panel}>
      ...
      <Space direction="vertical" style={{ width: "100%" }}>
        <Typography.Text>{task.status} · 版本 {task.version}</Typography.Text>
