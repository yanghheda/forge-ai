...
import { Alert, Button, Card, Input } from "@arco-design/web-react";
...
export function RequirementMaterials({
  ...
  if (!editing) {
    return (
      <Card
        title="需求材料"
        extra={<Button onClick={() => setEditing(true)}>编辑需求</Button>}
      >
          ...
          <div>
            ...
            <dd>
              {details.acceptanceCriteria.length > 0
                ? details.acceptanceCriteria.map((item) => <div key={item}>• {item}</div>)
    ...
    <Card title="需求材料">
      <div className={styles.form}>
        <MaterialField
          label="业务目标"
          hint="需求要达成的业务成果"
          placeholder="例：让新用户 3 分钟内完成首次需求提交"
          minRows={2}
          maxRows={4}
          value={form.goal}
      ...
      {save.isError && (
        <Alert
          ...
          type="error"
          content={formatRequestError(save.error)}
        />
      )}
    </Card>
  );
