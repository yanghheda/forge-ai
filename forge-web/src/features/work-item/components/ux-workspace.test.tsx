...
describe("UX organization", () => {
  it("将 UX Task 渲染为可进入详情的链接", () => {
    ...
    expect(screen.getByRole("link", { name: /FORGE-2.*测试需求.*TODO/ })).toHaveAttribute("href", "/w/personal/p/FORGE/ux/2");
  });

  it("按照服务端 availableActions 推进 UX Task", () => {
    const transition = vi.fn();
    render(<UxTaskDetailView task={{ ...task, availableActions: ["START"], guardHints: {} }} busy={false} onTransition={transition} />);
