...
export function DeliveryGraphView({ graph }: { graph: Graph }) {
  ...
  return (
    <Card title="交付关系图">
      {graph.truncated && <Alert type="warning" content={`结果已按深度 ${graph.maxDepth}、节点 ${graph.maxNodes} 的上限截断。`} />}
      {!useListOnly && (
        <svg aria-label="交付关系图" role="img" viewBox={`0 0 ${Math.max(480, (maxDepth(graph) + 1) * 220)} ${Math.max(120, graph.nodes.length * 72)}`} style={{ width: "100%", minHeight: 180 }}>
          {graph.edges.map((edge) => {
            const source = positions.get(edge.source);
            const target = positions.get(edge.target);
            return source && target ? <line key={edge.id} x1={source.x + 80} y1={source.y} x2={target.x - 80} y2={target.y} stroke="currentColor" strokeOpacity="0.35" /> : null;
          })}
          {graph.nodes.map((node) => {
            ...
            return (
              <g key={node.id} transform={`translate(${position.x}, ${position.y})`}>
                ...
                <text textAnchor="middle" y="13" fontSize="10">
                  {shorten(node.title)}
                </text>
              </g>
            );
      ...
      {useListOnly && <Alert type="info" content="节点较多，已切换为列表视图以保持页面可用。" />}
