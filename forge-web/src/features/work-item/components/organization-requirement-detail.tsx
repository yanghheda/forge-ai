...
import { Alert, Button, Card, Form, Message, Select, Spin, Tag, Typography } from "@arco-design/web-react";
...
export function OrganizationRequirementDetail({ requirementId }: { requirementId: number }) {
  ...
  const requirement = useQuery({
    ...
    queryFn: () => getOrganizationRequirement(requirementId),
  });
  const participants = useQuery({
    queryKey: ["requirements", requirementId, "participants"],
    queryFn: () => getRequirementParticipants(requirementId),
  });
  const members = useQuery({
    queryKey: ["requirements", "members"],
    queryFn: () => listRequirementMembers(),
  });
    ...
    <section className={styles.page}>
      ...
      <header className={styles.header}>
        <div>
          ...
          <Typography.Paragraph>{item.description || "尚未补充需求描述。"}</Typography.Paragraph>
        </div>
        <div>
          <Tag color="arcoblue">{item.status}</Tag>
          <Tag>{priorityLabel(item.priority)}优先级</Tag>
        </div>
      </header>
      <div className={styles.grid}>
        <Card title="需求协作人员" className={styles.people}>
          <p>按需求关联产品、UX、开发和测试人员。首个 Owner 无需额外关联即可执行全部流程。</p>
          <Form layout="vertical" onSubmit={() => save.mutate()}>
                ...
                <Form.Item key={role} label={roleLabel(role)}>
                  <Select
                    ...
                    options={(members.data ?? [])
                      .filter((member) => member.roles.includes(role) || member.roles.includes("OWNER"))
                      .map((member) => ({
                        value: member.userId,
