...
import { completeDevTask, createDevTask, getDeliveryGraph, getDevelopmentSummary, startDevelopment } from "./work-item-api";
...
describe("work item API", () => {
  it("按 Organization、Project 和 Requirement 范围读取 Delivery Graph", async () => {
    ...
    expect(client.request).toHaveBeenCalledWith("/v1/work-items/30/delivery-graph?organizationId=10&organizationId=20");
  });
  ...
  it("读取开发汇总并用任务版本完成 Dev Task", async () => {
    ...
    expect(client.request).toHaveBeenNthCalledWith(1, "/v1/development/requirements/30?organizationId=10&organizationId=20");
