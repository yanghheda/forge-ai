...
export function WorkspaceProjects({ workspaceSlug }: { workspaceSlug: string }) {
  ...
  return <section aria-labelledby="workspace-projects-title">
    <div className={styles.heading}><div><span className={styles.eyebrow}>WORKSPACE</span><h1 id="workspace-projects-title">{workspace.name}</h1><p>集中管理产品交付、团队协作与智能 Agent。</p></div>{canManageProjects && <Button type="primary" icon={<IconPlus />} onClick={() => setCreating((value) => !value)}>{creating ? "取消创建" : "新建项目"}</Button>}</div>
    <div className={styles.stats}><div><small>全部项目</small><strong>{projects.data.length}</strong></div><div><small>活跃项目</small><strong>{active}</strong></div><div><small>我的角色</small><strong className={styles.role}>{workspace.roles.join(" · ")}</strong></div></div>
    {creating && <form onSubmit={submit} className={styles.createForm}><div className={styles.formTitle}><div><h2>创建新项目</h2><p>项目 Key 创建后将用于工作项编号与路由。</p></div></div><div className={styles.fields}><label>项目 Key<Input aria-label="项目 Key" placeholder="例如 FORGE" value={form.key} onChange={(key) => setForm((value) => ({ ...value, key: key.toUpperCase() }))} maxLength={12} /></label><label>项目名称<Input aria-label="项目名称" placeholder="面向团队的项目名称" value={form.name} onChange={(name) => setForm((value) => ({ ...value, name }))} /></label><label className={styles.description}>项目说明<Input aria-label="项目说明" placeholder="简要说明目标和范围（可选）" value={form.description} onChange={(description) => setForm((value) => ({ ...value, description }))} /></label></div><div className={styles.formActions}>{create.isError && <RequestError error={create.error} />}<Button onClick={() => setCreating(false)}>取消</Button><Button htmlType="submit" type="primary" disabled={!form.key.trim() || !form.name.trim()} loading={create.isPending}>创建 Project</Button></div></form>}
...
export function RequestError({ error }: { error: unknown }) { const requestId = isApiError(error) ? error.requestId : undefined; return <Alert type="error" content={requestId ? `请求失败。Request ID: ${requestId}` : "请求失败，请稍后重试。"} />; }
