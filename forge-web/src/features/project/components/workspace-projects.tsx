...
import { Alert, Button, Empty, Input, Message, Spin, Tag } from "@arco-design/web-react";
...
export function WorkspaceProjects({ workspaceSlug }: { workspaceSlug: string }) {
  ...
  const create = useMutation({ mutationFn: () => createProject({ workspaceId: workspace!.id, ...form }), onSuccess: () => { Message.success("项目创建成功。"); setForm({ key: "", name: "", description: "" }); setCreating(false); void queryClient.invalidateQueries({ queryKey: ["projects", workspace?.id] }); }, onError: (error) => Message.error(formatRequestError(error)) });
  ...
  return <section aria-labelledby="workspace-projects-title">
    ...
    {creating && <form onSubmit={submit} className={styles.createForm}><div className={styles.formTitle}><div><h2>创建新项目</h2><p>项目 Key 创建后将用于工作项编号与路由。</p></div></div><div className={styles.fields}><label>项目 Key<Input aria-label="项目 Key" placeholder="例如 FORGE" value={form.key} onChange={(key) => setForm((value) => ({ ...value, key: key.toUpperCase() }))} maxLength={12} /></label><label>项目名称<Input aria-label="项目名称" placeholder="面向团队的项目名称" value={form.name} onChange={(name) => setForm((value) => ({ ...value, name }))} /></label><label className={styles.description}>项目说明<Input aria-label="项目说明" placeholder="简要说明目标和范围（可选）" value={form.description} onChange={(description) => setForm((value) => ({ ...value, description }))} /></label></div><div className={styles.formActions}><Button onClick={() => setCreating(false)}>取消</Button><Button htmlType="submit" type="primary" disabled={!form.key.trim() || !form.name.trim()} loading={create.isPending}>创建项目</Button></div></form>}
