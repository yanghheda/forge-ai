...
export function ProtectedWorkspace({ workspace, children }: { workspace: string; children: ReactNode }) {
  ...
  if (!access) return <Alert type="error" content="当前账户无权访问此工作空间。" />;

  return <section>
    <Card size="small">
      <Typography.Text>{currentUser.data.displayName} · {access.name} · {access.roles.map(roleLabel).join("、")}</Typography.Text>
