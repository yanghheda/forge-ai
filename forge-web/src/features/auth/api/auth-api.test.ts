...
import { getCurrentUser, getSetupStatus, initializeInstance, login, logout } from "./auth-api";
...
describe("auth api", () => {
  it("所有身份写操作都声明非安全 HTTP 方法以触发 CSRF transport", async () => {
    ...
    await initializeInstance({
      ...
      organizationName: "Forge", organizationSlug: "forge", workspaceName: "Engineering", workspaceSlug: "engineering",
    }, client);
    ...
    expect(client.request.mock.calls.map(([path, init]) => [path, init?.method])).toEqual([
      ["/v1/setup/status", undefined], ["/v1/setup/initialize", "POST"], ["/v1/auth/login", "POST"],
