...
cleanup() {
  docker compose --profile applications --env-file "$environment_file" -f "$compose_file" down
}
trap cleanup EXIT INT TERM

docker compose --profile applications --env-file "$environment_file" -f "$compose_file" up -d --build --wait
...
docker compose --profile applications --env-file "$environment_file" -f "$compose_file" \
